...
function [] = createSkyandSunGIF(theta0)
%% Define constants
...
for aa = 1:length(theta0)
    ...
    % ------ Solar Flux at Observer along Solar Zenith Angle -----
    % Lets compute the solar radiance at the observer, after travelling
    % through the slant path L0
    
    % Compute the altitude above the surface along the line of sight
    % looking at the sun
    
    D0 = 180 - theta0(aa);                              % deg
    
    phi = asind(Re*sind(D0)/(Re+H));                    % deg
    
    E0 = theta0(aa) - phi;                          % deg
    
    % values of z above the earths surface along the line of sight looking
    % at the sun
    E0_vec = linspace(E0/10,E0,10);                          %deg
    Z0 = Re*sind(D0)./sind(theta0(aa) - E0_vec) - Re;        % Altitude above the Earths Surface
    
    % Compute the number concentration along this path
    Nc_0 = interp1(altitude, air_Nc, Z0, 'spline');           % cm^(-3)
    
    % Compute the transmission
    T_obs = zeros(N_W,1);
    for ww = 1:N_W
        T_obs(ww) = exp(-trapz(Z0,(Nc_0.*sigma_e(ww))*1e5));
    end
    
    % compute the direct solar radiance
    solar_radiance_obs = solar_radiance.*T_obs;
    % -------------------------------------------------------
    ...
    for tt = 1:N_T
        ...
        RGB_sky(:,tt) = uint8(255*(RGB./max(RGB)));
    ...
    % --- Compute for the Sun -----
    % Lets determine the CIE-XYZ color values
    X0 = trapz(wl(index), x_funcT.*solar_radiance_obs(index));
    Y0 = trapz(wl(index), y_funcT.*solar_radiance_obs(index));
    Z0 = trapz(wl(index), z_funcT.*solar_radiance_obs(index));
    
    
    % We know normalize these values
    xyz0 = [X0/(X0+Y0+Z0), Y0/(X0+Y0+Z0), Z0/(X0+Y0+Z0)];
    
    RGB0 = color_mat*xyz0';
    RGB_sun = uint8(255*(RGB0./max(RGB0)));
    ...
    sz = size(repmat(reshape(RGB_sky',N_T,1,3),1,px_cols,1));
    xg = 1:sz(1);
    yg = 1:sz(2);
    interpGrid = griddedInterpolant({xg,yg},double(repmat(reshape(RGB_sky',N_T,1,3),1,px_cols,1)));
    ...
    if theta0(aa)<=90
        ...
        rectangle('Position',pos,'Curvature',[1 1], 'FaceColor',RGB_sun, 'EdgeColor',[0,0,0])
        axis equal
